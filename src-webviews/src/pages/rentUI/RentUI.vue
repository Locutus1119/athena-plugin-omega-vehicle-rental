...
export default defineComponent({
    ...
    mounted() {
        ...
        // this.fillRentVehicles(RENT); // Debugging Purpose
        if ('alt' in window) {
            alt.emit(`${ComponentName}:Ready`);
            alt.on(`${ComponentName}:Vue:SetVehicles`, this.fillRentVehicles, );
        }
    ...
    unmounted() {
        ...
        if ('alt' in window) {
            ...
            alt.off(`${ComponentName}:Vue:SetVehicles`, this.fillRentVehicles);
        }
    ...
    computed: {
        filteredVehicles() {
            return this.RentSystem.RentVehicles.filter((RentVehicle) =>
                RentVehicle.name.toLowerCase().includes(this.search.toLowerCase()),
            );
        },
        cssVars() {
            return {
    ...
    methods: {
        ...
        fillRentVehicles(rentVehicles: Array<String | number>[], type: string,) {
            const rentSystem = { ...this.RentSystem };
...
.logo {
    ...
    height: auto;
    min-height: 222px;
    background: url('./orslogo.png') no-repeat;
